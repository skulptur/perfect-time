...
export function ADSRNode(ctx: AudioContext, opts: Partial<EnvolopeOptions>) {
  function getNum(opts, key, def) {
    if (typeof def === 'number' && typeof opts[key] === 'undefined') return def
  ...
  function curveValue(type, startValue, endValue, curTime, maxTime) {
    if (type === 0) return startValue + (endValue - startValue) * Math.min(curTime / maxTime, 1)
  ...
  function adjustCurve(type, startValue, endValue) {
    ...
    if (type === 0) return endValue // linear hits its target, so no worries
  ...
  function triggeredValue(time) {
    ...
    if (time < atktime) {
      return curveValue(acurve, lastTrigger.v, adjustCurve(acurve, lastTrigger.v, peak), time, atktime)
    }
    if (time < atktime + hold) return peak
    if (time < atktime + hold + decay) return curveValue(dcurve, peak, sustain_adj, time - atktime - hold, decay)
    return sustain
  }
  ...
  function releasedValue(time) {
    ...
    if (time < 0) return sustain
    if (time > lastRelease.reltime) return base
    return curveValue(rcurve, lastRelease.v, adjustCurve(rcurve, lastRelease.v, base), time, lastRelease.reltime)
  }

  function curveTo(param, type, value, time, duration) {
    if (type === 0 || duration <= 0) param.linearRampToValueAtTime(value, time + duration)
    // exponential
    else param.setTargetAtTime(value, time, duration / type)
  }

  node.trigger = function(when = this.context.currentTime) {
    if (lastTrigger !== false) {
      if (when < lastTrigger.when) throw new Error('[ADSRNode] Cannot trigger before future trigger')
      this.release(when)
    }
    var v = base
    ...
    var atktime = attack
    if (Math.abs(base - peak) > eps) atktime = (attack * (v - peak)) / (base - peak)
    ...
    if (DEBUG) {
      ...
      for (var i = 0; i < 10; i += 0.01) this.offset.setValueAtTime(triggeredValue(i), when + i)
      return this
    }

    if (interruptedLine) this.offset.linearRampToValueAtTime(v, when)
    else this.offset.setTargetAtTime(v, when, 0.001)
    curveTo(this.offset, acurve, adjustCurve(acurve, v, peak), when, atktime)
    this.offset.setTargetAtTime(peak, when + atktime, 0.001)
    if (hold > 0) this.offset.setTargetAtTime(peak, when + atktime + hold, 0.001)
  ...
  node.release = function(when) {
    if (typeof when === 'undefined') when = this.context.currentTime

    if (lastTrigger === false) throw new Error('[ADSRNode] Cannot release without a trigger')
    if (when < lastTrigger.when) throw new Error('[ADSRNode] Cannot release before the last trigger')
    var tnow = when - lastTrigger.when
    var v = triggeredValue(tnow)
    var reltime = release
    if (Math.abs(sustain - base) > eps) reltime = (release * (v - base)) / (sustain - base)
